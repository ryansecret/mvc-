 ...
 */
	...
	$.jstree.plugins.checkbox = function (options, parent) {
		this.bind = function () {
		    ...
		    if (!this.settings.checkbox.cascade) {
		        if (this.settings.checkbox.three_state&&!this.settings.checkbox.ryan) {
		    ...
		    if (this.settings.checkbox.ryan) {
		        this.element.on(this.settings.checkbox.tie_selection ? 'select_node.jstree' : 'check_node.jstree', $.proxy(function (e, data) {
		            if (this.settings.checkbox.cascade.indexOf('down') == -1) {
		                return;
		            }
		            var obj = data.node,
                        m = this._model.data,
		        ...
		        }, this)).on('changed.jstree uncheck_node.jstree check_node.jstree uncheck_all.jstree check_all.jstree move_node.jstree copy_node.jstree redraw.jstree open_node.jstree', $.proxy(function () {
		            // only if undetermined is in setting
		            if (this._data.checkbox.uto) { clearTimeout(this._data.checkbox.uto); }
		            this._data.checkbox.uto = setTimeout($.proxy(this._undetermined, this), 50);
		        }, this));;
