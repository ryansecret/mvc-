...
namespace Hsr.Core.Plugins
{
    ...
    public class PluginManager
    {
        ...
        public static void Initialize()
        {
            ...
            using (new WriteLockDisposable(Locker))
            {
                ...
                try
                {
                    ...
                    var desFiles = GetDescriptionFilesAndDescriptors(pluginFolder);
                    logger.Info("有"+desFiles.Count()+"个文件");
                    //load description files
                    foreach (var dfd in desFiles)
                    {
                        var descriptionFile = dfd.Key;
                        var pluginDescriptor = dfd.Value;

                        //ensure that version of plugin is valid
                        if (!pluginDescriptor.SupportedVersions.Contains(HsrVersion.CurrentVersion, StringComparer.InvariantCultureIgnoreCase))
                        {
                        ...
                        try
                        {
                            ...
                            pluginDescriptor.OtherReferenceAssembly = pluginFiles;
                            pluginDescriptor.OriginalAssemblyFile = mainPluginFile;
                            //var assemblys=  AppDomain.CurrentDomain.GetAssemblies();
                            //var dd= assemblys.Where(d => d.FullName.Contains(pluginDescriptor.PluginFileName));
                            //installed assembly load in the domain
                            if (pluginDescriptor.Installed)
                            {
                                ...
                                foreach (var plugin in pluginFiles
                                    .Where(
                                        x =>
                                            !x.Name.Equals(mainPluginFile.Name,
        ...
        public static void LoadAssembly(PluginDescriptor pluginDescriptor)
        {
            pluginDescriptor.ReferencedAssembly = PerformFileDeployDynamic(pluginDescriptor.OriginalAssemblyFile);

            foreach (var plugin in pluginDescriptor.OtherReferenceAssembly
                                 .Where(
                                     x =>
                                         !x.Name.Equals(pluginDescriptor.OriginalAssemblyFile.Name,
                                             StringComparison.InvariantCultureIgnoreCase))
                                 .Where(x => !IsAlreadyLoaded(x)))
                PerformFileDeployDynamic(plugin);
            //init plugin type (only one plugin per assembly is allowed)
            foreach (var t in pluginDescriptor.ReferencedAssembly.GetTypes())
                if (typeof(IPlugin).IsAssignableFrom(t))
                    if (!t.IsInterface)
        ...
        public static IEnumerable<KeyValuePair<FileInfo, PluginDescriptor>> GetDescriptionFilesAndDescriptors(DirectoryInfo pluginFolder)
        {
        ...
        public static bool IsAlreadyLoaded(FileInfo fileInfo)
        {
        ...
        public static Assembly PerformFileDeploy(FileInfo plug)
        {
        ...
        public static Assembly PerformFileDeployDynamic(FileInfo plug)
        {
            if (plug.Directory.Parent == null)
                throw new InvalidOperationException("The plugin directory for the " + plug.Name +
                                                    " file exists in a folder outside of the allowed folder heirarchy");
            ...
            var shadowCopiedAssembly=  AppDomain.CurrentDomain.Load(AssemblyName.GetAssemblyName(shadowCopiedPlug.FullName));
            return shadowCopiedAssembly;
        }
        /// <summary>
        ...
        /// <returns></returns>
        public static FileInfo InitializeFullTrust(FileInfo plug, DirectoryInfo shadowCopyPlugFolder)
        {
        ...
        /// <param name="shadowCopyPlugFolder"></param>
        /// <returns></returns>
        public static FileInfo InitializeMediumTrust(FileInfo plug, DirectoryInfo shadowCopyPlugFolder)
        {
            var shouldCopy = true;
            var shadowCopiedPlug = new FileInfo(Path.Combine(shadowCopyPlugFolder.FullName, plug.Name));
