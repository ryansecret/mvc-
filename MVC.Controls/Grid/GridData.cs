...
namespace MVC.Controls.Grid
{
    ...
    public class GridData
    {
        ...
        public GridData UseValuesFromColumnExpressions<T>(GridColumnModelList<T> columns) where T : class
        {
            ...
            Rows = objects.Select(row => columns.GenerateValueDictionary((T)row)).ToList();
            
            //foreach (object obj in Rows)
            //{
            //    Rows =
            //       objects
            //            .Select(row => columns.GenerateValueDictionary((T)row))
            //            .ToList();
            //}
